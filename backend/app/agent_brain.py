...
# 流程控制意图（安排考试金线）+ 业务查询意图（各大教学业务模块，只读）。
FLOW_INTENTS = {"arrange_exam", "confirm", "publish", "reset"}
QUERY_INTENTS = {
    "data_board",       # 数据看板
    "list_students",    # 学员名册
    "list_exams",       # 考试列表 / 监考
    "show_grading",     # 成绩 / 阅卷分析
    "list_questions",   # 题库
    "recommend_cases",  # 病例推荐
    "list_teaching",    # 教学计划
}
VALID_INTENTS = FLOW_INTENTS | QUERY_INTENTS | {"smalltalk"}
...
_INTENT_SYSTEM_PROMPT = """你是「巨鲨数字助教·鲨鲨」的意图路由器。你能驱动真实教学平台完成多项教学业务，
核心金线是「安排一场胸部 CT 基础诊断考试」：识别需求 → 生成方案 → 讲师确认 → 创建并预览试卷 →
下发考试 → 监控答题 → 自动阅卷 → 推荐复训病例。除此之外你还能查询教学平台的各大业务模块。
...
【业务查询意图】（只读，随时可问；当且仅当不是在确认/下发金线时才匹配）
- data_board：想看数据看板/整体概览/一共有多少考试、试卷、题目/运营统计。
- list_students：想看现场学员名单/在科学员/查某个学员/有多少人。
- list_exams：想看考试列表/有哪些考试/答题进度/监考/交卷情况。
- show_grading：想看成绩/阅卷结果/平均分/通过率/薄弱点/考试报告。
- list_questions：想看题库/有哪些题/多少道题/题型构成。
- recommend_cases：想要推荐复训病例/训练病例/下一阶段练什么。
- list_teaching：想看教学计划/教学阅片/排课/教学安排。

【其它】
- smalltalk：寒暄、提问、与上述业务都无关或信息不足。

重要：如果"当前演示状态"正处于等待方案确认或等待下发确认，且用户在表示同意/下发，
请判为 confirm 或 publish，不要误判成业务查询意图。
...
# 业务回答改写层：把「基于真实数据的确定性事实话术」按用户的具体问法自然改写。
# LLM 只负责润色，事实由 facts 提供，杜绝编造。
_ANSWER_REFINE_SYSTEM_PROMPT = """你是「巨鲨数字助教·鲨鲨」，医学教学展厅里的语音数字助教。
我会给你一个 JSON：user_question 是老师刚问的话，facts 是基于真实教学平台数据已经核实的事实要点。

请把 facts 用口语化、自然、专业的中文复述给老师听，使其更贴合 user_question 的问法（1~3 句，适合 TTS 朗读）：
- 用第一人称"我"，称呼老师用"您"，像现场助教自然接话。
- 只能使用 facts 里给出的数字与事实，绝不可新增、编造或夸大任何数据；不确定就不要说。
- 如果 facts 带"（演示数据）"前缀，说明这是演示兑底数据，不要谎称是真实平台数据。
- 不要使用：后台、系统、接口、模型、TTS、字段、JSON、event 等工程术语。

直接输出要朗读的话即可，不要使用 JSON、不要 Markdown、不要加引号或前缀。"""
...
class AgentBrain:
    ...
    async def classify_voice_intent(
        ...
        *,
        ...
        """语音关键路径的限时意图识别：超预算/失败返回 None，由网关回退正则路由。"""
        if not settings.chat_llm_configured or budget <= 0:
            return None
        task = asyncio.create_task(
            self.classify(
                message=message, state=state, need_confirm=need_confirm,
                confirmation_type=confirmation_type, default_plan=default_plan,
            )
        )
        try:
            ...
            task.cancel()
            logger.info("voice intent classify hit budget %.2fs; fallback to regex", budget)
            return None
        except Exception as exc:  # noqa: BLE001
            logger.warning("voice intent classify failed: %s", exc)
            return None

    async def refine_answer(
        self, *, user_text: str, facts: str, budget: float,
    ) -> str | None:
        """把确定性事实话术(facts)按用户的具体问法自然改写成口播；超时/失败返回 None。

        LLM 只做润色，事实由 facts 提供，杜绝编造。"""
        facts = (facts or "").strip()
        if not settings.chat_llm_configured or budget <= 0 or not facts:
            return None
        payload = {"user_question": user_text, "facts": facts}
        task = asyncio.create_task(
            self._chat_once(
                _ANSWER_REFINE_SYSTEM_PROMPT, json.dumps(payload, ensure_ascii=False)
            )
        )
        try:
            ...
            task.cancel()
            logger.info("answer refine hit budget %.2fs; using facts", budget)
            return None
        except Exception as exc:  # noqa: BLE001
            logger.warning("answer refine failed: %s", exc)
            return None
        text = (text or "").strip().strip("\"'“”")
        if not text:
            return None
        # 诚信底线：facts 标了「演示数据」而改写丢了标记，宁可回退 facts，绝不把演示数据说成真实。
        if "演示数据" in facts and "演示" not in text:
            return None
        return text
